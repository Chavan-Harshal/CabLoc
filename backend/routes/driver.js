...
router.post("/approve", async (req, res) => {
  ...
  let to_d = "";
  await connection.query(
    `INSERT INTO ongoing values("${trip_id}","${user_id}")`,
    (e, o) => {
      if (e) {
        console.log(e);
      }
  ...
  await connection.query(
    ...
    async (e, op) => {
      if (e) {
        ...
        to_d = op[0].to_d;
        await connection.query(
          `UPDATE trip2 SET duration="${duration}",fare=${fare} WHERE from_s="${from_s}" and to_d="${to_d}" and trip_id="${trip_id}";UPDATE trip4 SET status=TRUE WHERE trip_id="${trip_id}"`,
          [1, 2],
          (e, op) => {
            if (e) {
              console.log(e);
              ...
              connection.query(
                `INSERT INTO trip1 values("${start}","${end}","${duration}","${trip_id}")`,
                (e, o) => {
                  if (e) {
                    console.log(e);
                    return res.status(400).json({ msg: "err" });
